...
import { randomizeQuiz } from '../lib/quizRandom';
import {
  BackArrowIcon,
  BoostZapIcon,
  HintBulbIcon,
    ...
    async function fetchQuestions() {
      try {
        ...
        if (data.error) {
          ...
          // Shuffle question order + A/B/C/D so every attempt feels different
          const questions = randomizeQuiz(data.questions);
  ...
  const handleOverclock = async () => {
    ...
    try {
      if (isGuestProfile(profile)) {
        ...
        await updateDoc(doc(db, 'users', profile.id), { coins: increment(-10) });
      }
      ...
      <div className="flex min-h-dvh flex-col items-center justify-center bg-bg-main px-6 py-16 text-center">
        ...
        <div className="mt-8 max-w-sm space-y-4">
          ...
          <p className="text-sm leading-relaxed text-text-secondary">
            {error || "Wax baa qaldamay. Isku day mar kale."}
          </p>
          <div className="flex flex-col gap-3 sm:flex-row sm:justify-center">
            <button
              type="button"
              onClick={() => window.location.reload()}
              className="inline-flex h-12 items-center justify-center bg-brand-cyan px-8 font-display text-sm font-semibold text-[#0B1424] hover:brightness-110"
            >
              Isku day mar kale
            </button>
            <button
              ...
              className="inline-flex h-12 items-center justify-center border border-border-light px-8 font-display text-sm font-semibold text-text-primary hover:border-brand-cyan"
            >
              Ku noqo mawduucyada
            </button>
          </div>
  ...
  return (
    <div className="relative flex min-h-dvh flex-col overflow-hidden bg-bg-main">
      <div
        ...
        style={{
          background:
            'radial-gradient(ellipse 80% 45% at 50% -8%, rgba(0,194,255,0.11) 0%, transparent 55%)',
        }}
      />
      ...
      <header className="relative z-20 flex items-center justify-between gap-3 px-4 py-4 sm:px-6">
        ...
        <div className="flex items-center gap-2">
          {gameState.streak > 0 && (
            <div className="hidden items-center gap-1 border border-border-light bg-surface/60 px-2.5 py-2 text-amber-400 backdrop-blur-md sm:flex">
              <StreakFlameIcon size={14} />
              <span className="font-mono text-xs font-bold text-text-primary">{gameState.streak}</span>
            </div>
          )}
          <div className="flex items-center gap-1.5 border border-border-light bg-surface/60 px-3 py-2 text-amber-400 backdrop-blur-md">
            <TrophyIcon size={15} />
      ...
      <div className="relative z-20 mx-auto w-full max-w-2xl px-5 pb-3 sm:px-6">
        <div className="mb-1.5 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-text-secondary">
            <QuestionStackIcon size={18} className="text-brand-cyan" />
          ...
          >
            <TimerRingIcon size={18} progress={timerRatio} urgent={timerUrgent} />
      ...
      <main className="relative z-10 mx-auto flex w-full max-w-2xl flex-1 flex-col px-5 py-4 sm:px-6 sm:py-6">
        <AnimatePresence mode="wait">
          <motion.section
            ...
            className="mb-5 border border-border-light bg-surface/55 p-6 backdrop-blur-md sm:p-8"
          >
            ...
            {showExplanation && currentQuestion.explanation && (
              <motion.p
                ...
                className="mt-4 border-l-2 border-brand-cyan/70 pl-3 text-sm leading-relaxed text-text-secondary"
              >
          ...
          {currentQuestion.options.map((option, idx) => {
            ...
            return (
              <motion.button
                ...
                className={`flex w-full items-center gap-4 border p-4 text-left transition-colors disabled:cursor-default sm:p-5 ${stateClass}`}
              >
                <span
                  className={`flex h-10 w-10 shrink-0 items-center justify-center border font-display text-sm font-bold ${letterClass}`}
                >
      ...
      <footer className="relative z-20 mx-auto w-full max-w-2xl border-t border-border-light bg-surface/70 px-5 py-4 backdrop-blur-md sm:px-6">
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            <HintBulbIcon size={18} className="text-brand-cyan" />
          ...
          >
            <BoostZapIcon
              size={18}
              className={`text-brand-cyan ${isOverclocking ? 'animate-pulse' : ''}`}
            />
            <div className="text-left leading-tight">
              <span className="block font-display text-[11px] font-bold uppercase tracking-wider text-text-primary">
                {t.quiz.overclock}
